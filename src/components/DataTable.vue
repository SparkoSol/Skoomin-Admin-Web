  ...
  <v-card>
    <v-card-title>
      ...
      <div style="width: 400px; margin-right: 20px">
        <v-text-field v-model="search" solo label="Search anything..." dense hide-details />
      </div>
      <slot name="primary-action" />
      <v-btn v-if="allowAdd" color="primary" elevation="0" @click="$emit('add-new')">
    ...
    <v-data-table :loading="loading" :items="items" :headers="headersValue" :search="search"
      height="calc(100vh - 270px)">
      <template v-slot:item="{ item }">
        <tr>
          <td v-for="(elem, key) of headers" :key="key" :class="`text-${elem.align === 'right' ? 'end' : 'start'}`">
            <slot :name="elem.value" :item="item">{{ item[elem.value] }}</slot>
          </td>

          <td v-if="viewHandler || editHandler || deleteHandler || resetHandler" class="text-end">
            <slot name="extra-actions" :item="item" />

            <v-icon class="mx-2" v-if="viewHandler" small @click="viewHandler(item)">
              mdi-eye
            </v-icon>
            <v-icon  class="mx-2" v-if="editHandler" small @click="editHandler(item)" color="green">
              mdi-pencil
            </v-icon>
            <v-icon  class="mx-2" v-if="deleteHandler" small @click="onDelete(item)" color="red">
              mdi-delete
            </v-icon>

            <v-icon  class="mx-2" v-if="resetHandler" small @click="resetHandler(item)" color="red">
              mdi-restart
            </v-icon>
          </td>
        </tr>
